...
import {
  fetchAdminIssues, fetchWards, adminVerifyGrievance, adminCloseIssue, adminStartIssue, mediaUrl,
} from "@/lib/api";
...
export default function AdminScreen() {
  ...
  // Table rows: client-side status filter over the full set.
  const rows = useMemo(() => {
    if (!statusFilter) return issues;
    return issues.filter(i => i.status === statusFilter);
  }, [issues, statusFilter]);

  // Heatmap center = centroid of all geolocated issues.
  const center = useMemo(() => {
    const pts = issues.filter(i => i.latitude && i.longitude);
    ...
    <div className="flex h-screen flex-col bg-slate-100 text-slate-900">
      ...
      <section className="shrink-0 border-b border-slate-200 bg-white px-6 py-3" style={{ height: "42vh" }}>
        ...
        <div className="h-[calc(42vh-72px)] overflow-y-auto">
          {view === "overview" ? (
            <Overview counts={counts} total={total} issues={issues} />
          ) : (
            <div className="relative h-full overflow-hidden rounded-xl ring-1 ring-slate-200">
              {center ? (
                <AdminHeatMap issues={issues} mode={view} center={center} wards={wards} />
              ) : (
          ...
          <table className="w-full border-collapse text-sm">
            <thead className="sticky top-0 z-10 bg-slate-50 text-left text-[11px] font-bold uppercase tracking-wide text-slate-500">
              <tr>
                ...
                <th className="px-3 py-2 text-center">Ward</th>
    ...
    <>
      ...
      >
        ...
        <td className="px-3 py-2 text-center">
          <span className="rounded-full bg-violet-50 px-2 py-0.5 text-xs font-bold text-violet-700 ring-1 ring-violet-200">
            {issue.ward_no ?? "—"}
          </span>
        </td>
