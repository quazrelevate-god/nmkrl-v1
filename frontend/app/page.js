...
/**
 * Home — the citizen map screen (phase-1 pilot landing).
 *
 * This single scrollable screen replaces the retired Community feed as the app
 * home. Top-to-bottom it reads as clearly separated blocks:
 *   1. Brand bar + avatar (tap → citizen profile)  ← relocated from Community
 *   2. Constituency highlights bar                  ← relocated from Community
 *   3. The live grievance map (rounded card)
 *   4. In My Ward / My Reports grievance lists
 * The only chrome is the floating "+" report button (see BottomNav).
 */
...
import CitizenProfileHeader from "@/components/citizen/CitizenProfileHeader";
                ...
                <div key={step} className="flex flex-1 flex-col items-center">
                  <div className="flex w-full items-center">
                    ...
                    <div className={`flex h-4 w-4 shrink-0 items-center justify-center rounded-full text-[8px] ${
                      active ? "glow-dot bg-brand text-white ring-2 ring-brand-200"
                  ...
                  <span className={`mt-1 flex h-6 items-start justify-center text-center text-[6px] leading-tight ${active ? "font-bold text-brand" : "text-slate-400"}`}>{step}</span>
                </div>
              );
            })}
          </div>
...
export default function HomeScreen() {
  ...
  // Header state (relocated from the Community screen).
  ...
  // The ward comes from the real GCC boundary lookup (via LocationDetector →
  ...
  // Hold rendering until the auth check resolves (prevents a flash before
  // redirecting unauthenticated visitors to /login).
  if (!authed) return null;

  return (
    <MobileShell fullBleed>
      {/* Everything scrolls in one natural flow: header → highlights → map →
          grievances, with generous spacing so the blocks read as separate. */}
      <div className="no-scrollbar flex-1 overflow-y-auto overscroll-contain pb-28">
        ...
        <section className="px-3 pt-4">
          <div className="mb-2 flex items-center justify-between px-1">
            <h2 className="text-[15px] font-extrabold tracking-tight text-slate-900">Grievance map</h2>
            <span className="text-[11px] font-medium text-slate-400">Tap a pin for details</span>
          </div>
          <div className="relative h-72 overflow-hidden rounded-[26px] shadow-lg ring-1 ring-slate-200/70">
            <MapView
              center={center}
              ...
              {pendingVerify.length > 0 && (
                <div className="mb-3 space-y-2">
                  {pendingVerify.map(issue => (
                    <div key={issue.id} className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                      <div className="flex items-center gap-2 mb-2">
                        <SlidersHorizontal size={14} className="text-brand" />
                        <p className="text-xs font-bold text-slate-700">Action Required — {issue.title}</p>
                      </div>
                      ...
                      <div className="flex gap-2">
                        <button
                          ...
                          className="flex flex-1 items-center justify-center gap-1 rounded-lg bg-emerald-600 py-2 text-xs font-semibold text-white disabled:opacity-60"
                        >
                        ...
                        <button
                          ...
                          className="flex flex-1 items-center justify-center gap-1 rounded-lg border border-slate-300 bg-white py-2 text-xs font-semibold text-slate-600 disabled:opacity-60"
                        >
